...
\begin{equation}
\label{eq:up:n:ch}
	\sigma^{ch}_t=0.95\text{ ns}/\sqrt{\text{\#channels}}, \hspace{2cm} \sigma^{ly}_t>200\text{ ps}
\end{equation}
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.5\linewidth]{dpt_sim}
...
The key feature of the transversal variables are the measurements that are independent from the poorly understood nuclear effects. To prove that selected sample depends weakly on the theoretical models we estimated anti-neutrino energy resolution with various 2p2h normalization and with different models of the initial nucleon momentum. The results of these cross-check is shown in \autoref{fig:up:n:model}.
...
\begin{figure}[!ht]
  ...
  \end{minipage}
  \caption{Accuracy of the reconstructed anti-neutrino energy after applying the lever arm and $\delta p_T$ cuts. The dependence on the 2p2h normalization (a) and initial nucleon momentum model (b) is shown.}
\end{figure}
